...
Identifies the media source for this report, such as `system_media` or `music_sdk`. Admins can hide media display by this value in **Media Source Block Rules**. This only hides `metadata.media`; it does not affect the activity record itself. When the home page hover card is configured to show media source, this value is used as the source label.
