...
const UPDATE_SITE_SETTINGS_TOOL_DESCRIPTION = [
  ...
  'For `appMessageRules`, each item should be a grouped object like `{ processMatch, defaultText?, titleRules }`; `processMatch` uses the process name such as `explorer.exe`, and each title rule uses `{ mode: "plain" | "regex", pattern, text }` for matching `processTitle`.',
